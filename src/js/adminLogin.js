// adminLogin.js
// Handles: badge/email toggle, remember me, login submit,
...
function eye(btnId, inputId) {
  ...
  ══════════════════════════ */
  const savedId   = localStorage.getItem('admin_rm_id');
  const savedMode = localStorage.getItem('admin_rm_mode');
  if (savedId && savedMode) {
    if (savedMode === 'email') document.getElementById('btnEmail').click();
  ...
  async function doLogin() {
    ...
    if (remember) {
      localStorage.setItem('admin_rm_id',   id);
      localStorage.setItem('admin_rm_mode', loginMode);
    } else {
      localStorage.removeItem('admin_rm_id');
      localStorage.removeItem('admin_rm_mode');
    }
    ...
    try {
      ...
      if (data.success) {
        if (!data.password_changed) {
          // First login — show mandatory password change modal
  ...
  /* ══════════════════════════
     FIRST-LOGIN MODAL
     Backdrop + Escape blocked — admin MUST set password
  ══════════════════════════ */
  ...
  // backdrop closes forgot modal only (not first-login)
